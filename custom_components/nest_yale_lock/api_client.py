    ...
    async def refresh_state(self):
        ...
        while retries < max_retries:
            for base_url in self._candidate_bases():
                ...
                try:
                    # Read chunks and accumulate until we have enough data
                    # The observe stream is long-lived, so we can't use response.read()
                    async with asyncio.timeout(10):  # 10 second timeout
                        async with self.session.post(api_url, headers=headers, data=observe_payload) as response:
                            if response.status != 200:
                                ...
                                continue
                            # Accumulate chunks until we have enough for a complete message
                            full_response = bytearray()
                            async for chunk in response.content.iter_chunked(65536):
                                full_response.extend(chunk)
                                # Try parsing once we have enough data (at least 3KB typically needed)
                                if len(full_response) >= 3000:
                                    _LOGGER.debug("refresh_state accumulated %d bytes, attempting parse", len(full_response))
                                    locks_data = await self.protobuf_handler._process_message(bytes(full_response))
                                    if locks_data.get("yale") or locks_data.get("structure_id"):
